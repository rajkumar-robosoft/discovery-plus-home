...
const carouselContent = [
  ...
  const Carousel = () => {
    ...
    useEffect(() => {
        ...
        const id = setTimeout(() => setActiveContent(next), 5000);
        ...
        return (
            <React.Fragment>
                {carouselContent.map((item, index) => {
                    return (
                        (index === activeContent) && <div className="carousel" key={index}>
                            <Link url="javascript:;" rel="/" onClick={prevContent} className="nav-icon"><Image src={PrevIcon} alt="Previous" width="24"/></Link>
                            <CarouselDesc heading={item.heading} subHeading={item.subHeading} tag1={item.tag1} tag2={item.tag2} playBtnSrc={PlayButton} playText='Play' playBtnWidth='90' key={index}/>
                            <CarouselImage src={item.image.img} alt={item.image.imgAlt} width='880'  />
                            <Link url="javascript:;" rel="/" onClick={nextContent} className="nav-icon"><Image src={NextIcon} alt="Next" width="24"/></Link>
                        </div>
                    );
                })}
                ...
                {(carouselContent).map((indicator, index) => {
                    return (
                        <CarouselIndicator key={index} onClick={(e) =>{e.preventDefault(); currentContent(index)}} className={activeContent === index ? "carousel-indicator-active" : "carousel-indicator-inactive"}/>
                    );
                })}
                </div>
          </React.Fragment>
